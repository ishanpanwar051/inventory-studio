...
const HistorySelectionModal = ({ customer, onClose, onSelectOrderHistory, onSelectTransactionHistory, type = 'customer' }) => {
    const { state } = useApp();
    ...
    const orderHistoryTitle = isSupplier ? 'Purchase Order History' : getTranslation('orderHistory', state.currentLanguage);
    const orderHistoryDesc = isSupplier ? 'View all past purchase orders' : getTranslation('viewAllPastOrders', state.currentLanguage);
